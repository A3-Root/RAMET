...
void writeSatImages(arma_file_formats::cxx::OprwCxx& wrp, const int32_t& worldSize, std::filesystem::path& basePathSat, const std::string& worldName)
{
    ...
    if (rvmats.size() > 1) {
        ...
        try {
            ...
            // Write single full-resolution composite for Docker post-processing.
            // Variants (sat_dark, baked_sat, baked_sat_dark) and pyramid tiling
            // are handled externally by tools/render_sat.py.
            dst.write((basePathSat / "sat_full.png").string());
        }
