...
if (uiNamespace getVariable ["ramet_isDiagBuild", false]) then {
    diag_log text "[RAMET] diag binary detected - Intercept host init skipped";
...
if (_registerTypesResult) then {
    uiNamespace setVariable ["intercept_fnc_event", compileFinal preprocessFileLineNumbers "\z\root_amet\addons\intercept_core\event.sqf"];
} else {
    uiNamespace setVariable ["intercept_fnc_event", compileFinal ""];
};
